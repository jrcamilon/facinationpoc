...
export class NodejsApiService {
   ...
   static boxPrimaryData = [
     { key: 1, primary: 'Innovation', languageOf: 'Creativity', adjectives: 'Creative, Entrepreneurial, Visionary',
      overview: 'Push companies to innovate with creativity.', communication: 'Tweak traditional conversation.',
      fascination: 'Get creative.', otherLeaders: 'Madonna, Albert Einstein, Amelia Earhart, Frank Lloyd Wright, Betty White' },
     { key: 2, primary: 'Passion', languageOf: 'Relationship', adjectives: 'Engaging, Expressive, Intuitive',
      overview: 'Build connections with warmth and enthusiam.', communication: 'Immediately create an emotional connection.',
       fascination: 'Connect with emotion.', otherLeaders: 'Leonardo Da Vinci, Ronald Regan, Audrey Hepburn, Oprah Winfrey, George Lucas' },
     { key: 3, primary: 'Power', languageOf: 'Confidence', adjectives: 'Confident, Goal-Oriented, Decisive',
     overview: 'Communicate with authority and confidence.', communication: 'Speak with authority and control.',
     fascination: 'Lead with authority.',
     otherLeaders: 'Arianna Huffington, Tim Tebow,  Michael Bloomberg, Warren Buffett, Mark Zuckerberg' },
     { key: 4, primary: 'Prestige', languageOf: 'Excellence', adjectives: 'Respected,  Ambitious, Results-Oriented',
     overview: 'Earn respect by setting high standards.', communication: 'Seek results and respect. ', fascination: 'Set the standard.',
     otherLeaders: 'Heidi Klum, Enzo Ferrari,  Princess Diana, James Bond, Gordon Ramsay' },
     { key: 5, primary: 'Trust', languageOf: 'Stability', adjectives: 'Stable, Dependable, Familiar',
     overview: 'Earn loyalty as a constent and familiar presence.', communication: 'Bring even-keeled consistency.',
     fascination: 'Build loyalty.', otherLeaders: 'Walt Disney, Sam Walton, Dr. Oz, Clint Eastwood, Mother Theresa' },
     { key: 6, primary: 'Mystique', languageOf: 'Listening', adjectives: 'Observant, Independent, Logical',
     overview: 'Impress with analytical skills and thoughtful communication.', communication: 'Listen carefully and observe.',
      fascination: 'Listen with care. ', otherLeaders: 'Tina Fey, Johnny Depp, Malcolm Gladwell, Stephen Hawking, Michael Crichton' },
     { key: 7, primary: 'Alert', languageOf: 'Details', adjectives: 'Detailed, Organized, Proactive',
      overview: 'Keep people and projects on track by managing the details.', communication: 'Focus on the task at hand.',
      fascination: 'Protect the details.', otherLeaders: 'Ralph Nader, Mary Poppins, Chuck Norris, James Cameron, Upton Sinclair' }
   ];
   static archetypeData = [
     ...
     { boxkey: 31, adjectives: 'Inventive, Untraditional, Self-Propelled' },
     ...
     { boxkey: 43, adjectives: 'Respected, Competitive, Results-Oriented' },
     ...
     { boxkey: 51, adjectives: 'Curious, Adaptable, Open-Minded' },
  ...
  public infosolOrgs = new Subject<any>();
  // local variables
  ...
  getAllFiles(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSAllDataEndpoint + '/:' +
     `${NodejsApiService.matrixConFilter}` + '/:' + NodejsApiService.matrixOrgFilter);

  }
  getPrimaryDonutChartData(): Observable<any> {

    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSPrimaryDonutChartData +
      NodejsApiService.orgFilter + '/:' + `${NodejsApiService.conFilter}`);
  }
  getSecondaryDonutChartData(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSSecondaryDonutChartData +
       NodejsApiService.orgFilter + '/:' + `${NodejsApiService.conFilter}`);
  }
  getDormantDonutChartData(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSDormantDonutChartData +
       NodejsApiService.orgFilter + '/:' +  NodejsApiService.conFilter);
  }
  getConferenceOrganizations(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSConferenceOrganizations +
       NodejsApiService.conFilter);
  }
  ...
  getInfosolOrganizations(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSConferenceOrganizations + 'InfosolAcademy');

  }
  getGenderCounts(): Observable<any> {
    return this.http.get(this.vm3NodeJSEndpoint + this.nodeJSGenderCountData + NodejsApiService.conFilter +
       '/:' + NodejsApiService.matrixOrgFilter + '/:' + NodejsApiService.boxFilter);
  }
